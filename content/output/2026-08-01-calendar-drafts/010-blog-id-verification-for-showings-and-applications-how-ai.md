<!-- fila 10 | 2026-08-07 | Blog | P5 Compliance-aware AI | Ph2 Problem-aware | doc 1g84W5mMo7emxLh8lbL5QDxLIcUKz9TVKeBRbgCFEN_c -->
...
<p>ID verification matters more now because synthetic identity fraud has reached a scale that manual document checks cannot catch. TransUnion's H2 2025 report found that synthetic identities, fabricated by combining real and fictional information, now account for a significant and growing share of fraud attempts across financial and rental applications.</p>
...
<p>Leasey.AI's ID verification runs in two flows, both designed to verify identity without adding meaningful friction to the leasing funnel. The verification takes approximately 90 seconds on the prospect's phone.</p>


<p><strong>At showing.</strong> When a prospect books a tour through Liza (the AI leasing agent) or directly through any of 48+ syndicated marketplaces, they receive a link to verify their ID before the showing is confirmed. The verification uses government-issued ID plus a selfie. Liveness detection prevents photo-of-a-photo workarounds and pre-recorded video spoofing.</p>


<p><strong>At application.</strong> The same verification flow is embedded in the application form. The result attaches to the application file alongside the SingleKey screening report (for Canadian operators) or your standard US screening. This creates a single verified identity thread from first enquiry through lease signing.</p>


<p>The verification can be required (no ID equals no tour confirmed) or optional (a flag in the dashboard where the operator decides). Common practice is to require it for higher-priced units and make it optional for entry-level listings where friction tolerance is lower.</p>
...
<p>The takeaway for any vendor in the leasing stack: tools that infer protected or competitively sensitive characteristics and act on them create real liability. ID verification that simply confirms identity without surfacing inferences does not. The verification is binary. The decision stays with the operator.</p>
...
<p>ID verification is included in Leasey.AI's standard pricing of $1.50 to $5 per door per month. There is no separate charge for the verification flow, the liveness detection, or the integration with the showing confirmation and application workflows. The cost is absorbed into the platform because verification is not an upsell; it is a core part of a responsible leasing workflow.</p>


<h2>Frequently Asked Questions</h2>


<h3>How long does ID verification take for the prospect?</h3>
<p>The verification takes approximately 90 seconds on the prospect's phone. They scan a government-issued ID and take a selfie. Liveness detection confirms the person is physically present. The result (verified, not verified, or pending) appears in the dashboard immediately, with no manual review required from the leasing team.</p>


<h3>Can I make ID verification optional instead of required?</h3>
<p>Yes. The verification can be set to required (no verification equals no confirmed tour) or optional (a dashboard flag where the operator decides). Common practice is to require verification for higher-priced units and make it optional for entry-level listings where minimising friction is the priority.</p>


<h3>Does ID verification expose protected characteristics to the operator?</h3>
<p>No. Leasey.AI's ID verification is deliberately binary. The operator sees verified, not verified, or pending. The system does not surface name, ethnicity, age, or address differently based on the document's contents. This design keeps the tool fair-housing-compliant in both Canada and the United States.</p>


<h3>What types of ID does the system accept?</h3>
<p>The system accepts government-issued photo identification including driver's licences, passports, and national ID cards. The liveness detection component requires a real-time selfie to prevent photo-of-a-photo workarounds and pre-recorded video spoofing, ensuring the person presenting the ID is physically present.</p>


<h3>How does ID verification help with synthetic identity fraud?</h3>
<p>Synthetic identities combine real and fictional information to create fabricated personas. Leasey.AI's liveness detection and document verification catch these by requiring both a valid government ID and a real-time biometric match. According to TransUnion (H2 2025), synthetic fraud reached $3.3 billion in exposure, making automated verification essential.</p>


<h3>Does ID verification slow down the leasing funnel?</h3>
<p>Traditional verification added 10 to 20 minutes to the process and caused many prospects to walk away. Leasey.AI's verification takes 90 seconds and is embedded in the existing SMS confirmation flow. The prospect completes it on their phone before the tour, with no additional steps at the property.</p>


<p>If your leasing team has been wanting better trust at the door without slowing tours down, <a href="https://leasey.ai/get-started">schedule a call</a>.</p>
